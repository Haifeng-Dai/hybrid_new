...
for server_epoch in range(server_epochs):
    ...
    # for i in all_client:
    #     acc_server[i].append(eval_model(client_list[0], test_loader[i]))
    #     log.info(msg_test_server.format(server_epoch + 1, acc_server[i][-1]))

save_path = f'./tmp/fedavg_seed_{seed}_alpha_{alpha}_dataset_{dataset}_model_structure_{model_structure}/'
file_name = save_path + \

...
for server_epoch in range(args.server_epochs):
    for avg_epoch in range(args.server_epochs):
        # local train
        client_list_ = []
        # lr = lr_all[server_epoch // (server_epochs // 5)]
        lr = 1e-4
        for cid, client in enumerate(client_list):
            log.info(msg_local.format(avg_epoch + 1, cid + 1))
            client_ = client.cuda()
        ...
        for sid, clients in enumerate(server_client):
            ...
            log.info(msg_test_server.format(avg_epoch + 1, acc_))
        client_list = deepcopy(client_list__)

    server_model = aggregate(server_list)[0]
    for sid in range(args.n_server):
        acc_ = eval_model(server_model, test_loader[sid])
        acc_server[sid].append(acc_)
        log.info(msg_test_server.format(server_epoch + 1, acc_))

save_path = f'./res/fedavg1_seed_{args.seed}_' + \
